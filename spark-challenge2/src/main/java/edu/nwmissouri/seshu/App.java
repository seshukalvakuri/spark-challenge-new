...
public final class App {
  ...
  private static void process(final String fileName) {
        ...
        final SparkConf sparkConf = new SparkConf().setMaster("local").setAppName("Challenge");
        ...
        final JavaRDD<String> wordsFromFile = inputFile.flatMap(line -> Arrays.asList(line.split(" ")).iterator());

        // use Java API to map each word to an output pair
        final JavaPairRDD<String, Integer> countData = wordsFromFile.mapToPair(t -> new Tuple2(t, 1))
        ...
        final String outputFolder = "results";
